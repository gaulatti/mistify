...
async def unified_analysis(req: UnifiedAnalysisRequest, http_request: Request):
    ...
    for item_req in req.items:
        item = UnifiedAnalysisItemResponse(text=item_req.text)
        detected_language = None

        if item_req.detect_language and app_state.fasttext_model:
            try:
                lang_req = LanguageDetectionRequest(text=item_req.text, k=item_req.language_count)
        ...
        if item_req.translate_to_english and app_state.translator:
            try:
                if not detected_language or detected_language.lower() not in ['en', 'eng']:
                    trans_req = TranslationRequest(text=item_req.text, source_language=detected_language)
                    trans_resp = await translate_text(trans_req, http_request)
                    item.translation = trans_resp
                else:
                    item.translation = TranslationResponse(
                        original_text=item_req.text,
                        translated_text=item_req.text,
        ...
        if item_req.classify_content and app_state.classifier:
            try:
                text_to_classify = item.translation.translated_text if item.translation else item_req.text
                class_req = ClassificationRequest(text=text_to_classify, labels=item_req.classification_labels)
